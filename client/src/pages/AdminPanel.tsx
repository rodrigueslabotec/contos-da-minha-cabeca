      ...
      <div className="container py-10">
        ...
        {stats && (
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
            {[
              ...
              { label: "Livros Publicados", value: stats.totalBooks, icon: BookOpen, color: "text-emerald-600" },
              { label: "Leituras Totais", value: stats.totalViews.toLocaleString(), icon: Eye, color: "text-purple-600" },
              { label: "Pendentes", value: stats.pendingBooks, icon: Clock, color: "text-amber-600" },
        ...
        <Tabs defaultValue="pending">
          <TabsList className="mb-6 flex-wrap h-auto gap-1">
            <TabsTrigger value="pending" className="gap-1.5">
              ...
              {pendingBooks && pendingBooks.length > 0 && (
                <span className="ml-1 h-5 min-w-5 rounded-full bg-amber-500 text-white text-xs flex items-center justify-center px-1">
                  {pendingBooks.length}
                </span>
              )}
            </TabsTrigger>
          ...
          <TabsContent value="pending">
            {pendingBooks && pendingBooks.length > 0 ? (
              <div className="space-y-4">
                {pendingBooks.map((book) => (
                  <div key={book.id} className="p-5 rounded-xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/10">
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-2 mt-2">
                            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                              book.contentRating === "18+" ? "bg-red-100 text-red-700" :
                              book.contentRating === "14+" ? "bg-amber-100 text-amber-700" :
              ...
              <div className="text-center py-16">
                <CheckCircle className="h-12 w-12 text-emerald-500/40 mx-auto mb-4" />
            ...
            {analysisResult && (
              <div className="mt-8 p-6 rounded-xl border-2 border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-900/10">
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-white dark:bg-card rounded-lg p-4">
                    <h4 className="font-semibold text-sm mb-2 flex items-center gap-2">
                      <BookMarked className="h-4 w-4 text-blue-600" /> Resumo do Conteúdo
                    </h4>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-white dark:bg-card rounded-lg p-4">
                      <h4 className="font-semibold text-sm mb-2 flex items-center gap-2">
                        <AlertTriangle className="h-4 w-4 text-amber-600" /> Classificação Etária
                      </h4>
                      <div className="flex items-center gap-2">
                        <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                          analysisResult.analysis.classificacaoValidada === "Adequada" ? "bg-emerald-100 text-emerald-700" :
                          analysisResult.analysis.classificacaoValidada === "Muito Restritiva" ? "bg-blue-100 text-blue-700" :
                    ...
                    <div className="bg-white dark:bg-card rounded-lg p-4">
                      <h4 className="font-semibold text-sm mb-2 flex items-center gap-2">
                        <TrendingUp className="h-4 w-4 text-purple-600" /> Qualidade Editorial
                      </h4>
                      <div className="flex items-center gap-2">
                        <div className="flex-1 bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                          <div
                            className="bg-gradient-to-r from-amber-400 to-emerald-500 h-2 rounded-full"
                  ...
                  {analysisResult.analysis.errosGramaticais && analysisResult.analysis.errosGramaticais.length > 0 && (
                    <div className="bg-white dark:bg-card rounded-lg p-4">
                      <h4 className="font-semibold text-sm mb-2 flex items-center gap-2">
                        <XCircle className="h-4 w-4 text-red-600" /> Erros Encontrados ({analysisResult.analysis.errosGramaticais.length})
                      </h4>
                  ...
                  {analysisResult.analysis.recomendacoes && analysisResult.analysis.recomendacoes.length > 0 && (
                    <div className="bg-white dark:bg-card rounded-lg p-4">
                      <h4 className="font-semibold text-sm mb-2 flex items-center gap-2">
                        <CheckCircle className="h-4 w-4 text-emerald-600" /> Recomendações
                      </h4>
                      <ul className="space-y-1 text-sm text-muted-foreground">
                    ...
                    <div key={book.id} className="flex items-center gap-4 p-4 rounded-xl border border-border bg-card">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-2 mt-1">
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusColors[book.status] ?? "bg-gray-100 text-gray-700"}`}>
                  ...
                  <div key={u.id} className="flex items-center gap-4 p-4 rounded-xl border border-border bg-card">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {u.isBanned && (
                          <span className="text-xs px-1.5 py-0.5 rounded bg-red-100 text-red-700">Banido</span>
                        )}
                      </div>
          ...
          <div className="space-y-4 py-2">
            {analysisResult && (
              <div className="p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
                <p className="text-xs font-semibold text-amber-700 dark:text-amber-400 mb-2">💡 Feedback da IA:</p>
                <p className="text-sm text-amber-700 dark:text-amber-300">{analysisResult.analysis.recomendacoes}</p>
              </div>
            )}
            <div>
